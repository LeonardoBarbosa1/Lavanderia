    ...
    <app-layout>
        ...
        >
            <div class="container">
                ...
                <pagination class="mt-6" :links="servicos.links" />
            </div>
        </div>
    </app-layout>
</template>
    ...
    methods: {
        destroy(servico) {
            if (confirm("Você tem certeza que deseja excluir este registro?")) {
                this.$inertia.delete(
                    this.route("servicos.destroy", servico.id)
                );
            }
        },
        search() {
            this.$inertia.replace(
                this.route("servicos.index", { term: this.term })
            );
        },
    },
};
